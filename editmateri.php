    ...
    if (!empty($_POST)) {
        ...
        if ($valid) {
            if(($picture != null) and ($file != null)){
              ...
              $sql = "Select * From post_materi WHERE id = $id";
              foreach ($pdo->query($sql) as $row) {
                  unlink("gambar/materi/".$row['Gambar']);
                  unlink("file/".$row['File']);
              }
              Database::disconnect();
              $pdo = Database::connect();
              $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $sql = "UPDATE post_materi set Nama = ?, Topik = ?,Email = ?, Gambar = ?, Pesan =?, File = ? WHERE id = ?";
              $q = $pdo->prepare($sql);
              $q->execute(array($name,$topic,$phone,$email,$picture,$message,$file,$id));
              ...
              $pdo = Database::connect();
              $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $sql = "UPDATE post_materi set Nama = ?, Topik = ?,Email = ?, Pesan =?, File = ? WHERE id = ?";
              $q = $pdo->prepare($sql);
              $q->execute(array($name,$topic,$phone,$email,$message,$file,$id));
              $ext = pathinfo($_FILES['fileMateri']['name'], PATHINFO_EXTENSION);
              if (($ext== "zip") or ($ext=="rar") ) {
                  move_uploaded_file($_FILES['fileMateri']['tmp_name'],"file/".$file);
              }
              ...
              $pdo = Database::connect();
              $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $sql = "UPDATE post_materi set Nama = ?, Topik = ?,Email = ?, Gambar = ?, Pesan =? WHERE id = ?";
              $q = $pdo->prepare($sql);
              ...
              $pdo = Database::connect();
              $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $sql = "UPDATE post_materi set Nama = ?, Topik = ?,Email = ?, Pesan =? WHERE id = ?";
              $q = $pdo->prepare($sql);
              $q->execute(array($name,$topic,$phone,$email,$message,$id));
              Database::disconnect();
            }
            header("Location: materikuliah.php");
        }
    } else {
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM post_materi where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Edit Materi</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel-body">
                        ...
                        <div class="row">
            <div class="col-md-8">
                <form name="sentMessage" id="contactForm" action="editmateri.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="control-group form-group">
                        <div class="controls">
                            ...
                            <input type="email" class="form-control" id="email" name="email" required data-validation-required-message="Please enter your email address." value="<?php echo !empty($email)?$email:'';?>">
					          ...
					          <div class="control-group form-group">
                        <div class="controls">
                            <img width=200 height=100 src="gambar/materi/<?php echo !empty($picture)?$picture:'';?>"><br>
                    ...
                    <div class="control-group form-group">
                        <div class="controls">
                            <strong><?php echo !empty($file)?$file:'';?></strong><br>
                            <label>Change File ( .rar ) :</label>
