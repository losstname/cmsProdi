            ...
            <div class="col-md-8">
                <form name="sentMessage" id="contactForm" method="post" enctype="multipart/form-data" novalidate>
                    <div class="control-group form-group">
                        ...
                        <div class="controls">
                            <label>Alamat:</label>
                            <input type="text" class="form-control" id="alamat" name="alamat" required data-validation-required-message="Please enter your topic.">
                        </div>
                    </div>
					          <div class="control-group form-group">
                        <div class="controls">
                            <label>Bidang:</label>
                            <input type="text" class="form-control" id="bidang" name="bidang" required data-validation-required-message="Please enter your topic.">
                        </div>
                    </div>
                    <div class="control-group form-group">
                        <div class="controls">
  ...
  require 'database.php';
        ...
        if ($valid) {
            if($picture != null){
              ...
              $sql = "INSERT INTO profil_dosen (Nama,Bidang,Telepon,Email,Alamat,Gambar) values(?, ?, ?, ?, ?, ?)";
              $q = $pdo->prepare($sql);
              $q->execute(array($name,$phone,$bidang,$alamat,$email,$picture));
              move_uploaded_file($_FILES['gambarDosen']['tmp_name'],"gambar/dosen/".$picture);
              Database::disconnect();
            }else{
              $pdo = Database::connect();
              $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $sql = "INSERT INTO profil_dosen (Nama,Bidang,Telepon,Email,Alamat) values(?, ?, ?, ?, ?)";
              $q = $pdo->prepare($sql);
              $q->execute(array($name,$bidang,$phone,$email,$alamat));
              Database::disconnect();
            }
	        }
